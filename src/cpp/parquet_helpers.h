...
#define LIST_APPEND(LIST_BUILDER, TEMPLATE_TYPE, TYPE_CLASS, CPP_CLASS) \
    if constexpr(std::is_same<TEMPLATE_TYPE, CPP_CLASS>::value) { \
        auto vb = dynamic_cast<arrow::TYPE_CLASS##Builder*>(LIST_BUILDER->value_builder()); \
...
#define INNERINNERLIST_APPEND(LIST_BUILDER, INNERLIST_BUILDER, TEMPLATE_TYPE, TYPE_CLASS, CPP_CLASS)      \
	if constexpr(std::is_same<TEMPLATE_TYPE, CPP_CLASS>::value) {   \
		auto vb = dynamic_cast<arrow::TYPE_CLASS##Builder*>(INNERLIST_BUILDER->value_builder()); \
		for(auto v : val) {	\
            PARQUET_THROW_NOT_OK(LIST_BUILDER->Append()); \
            for(auto v2 : v) { \
    ...
    void fill(T val, arrow::ArrayBuilder* builder) {
        ...
        if constexpr(is_std_vector<T>::value) {
            ...
            if constexpr(is_std_vector<InnerType>::value) {
                auto list2_builder = dynamic_cast<arrow::ListBuilder*>(list_builder->value_builder());
                ...
                if constexpr(is_std_vector<InnerType2>::value) {
                    auto list3_builder = dynamic_cast<arrow::ListBuilder*>(list2_builder->value_builder());
                    typedef typename getType<InnerType2>::type InnerType3;
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, Boolean, bool)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, UInt8, uint8_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, UInt16, uint16_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, UInt32, uint32_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, UInt64, uint64_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, Int8, int8_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, Int16, int16_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, Int32, int32_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, Int64, int64_t)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, Float, float)
                    INNERINNERLIST_APPEND(list2_builder, list3_builder, InnerType3, Double, double)
                    {
                        throw std::logic_error("ERROR: Unsupported list (dim=3) type");
                    }
                } else
                {
